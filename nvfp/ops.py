...
def scaled_fp4_quant(
    ...
    """
    Quantize input tensor to FP4 and return quantized tensor and scale (swizzled layout).

    Implemented with ``pytorch_nvfp4_quantize`` (PyTorch + reciprocal CUDA); no Cutlass quant kernel.
    """
    ...
    output_scale = sw.view(torch.uint8).reshape(rounded_m, rounded_n // 4, 4).view(
        torch.int32
    )
...
def cutlass_scaled_fp4_mm(
    ...
    """FP4 scaled GEMM via ``MMAEngine.emulation_scaled_fp4_mm`` (no Cutlass CUDA kernel)."""
